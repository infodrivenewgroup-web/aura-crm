...
2. Токен берётся из переменной окружения `DYXLESS_TOKEN`, а при её отсутствии используется значение по умолчанию (тот же аккаунт/ключ): `<set-via-DYXLESS_TOKEN>`.
...
- **Ключ:** `DYXLESS_TOKEN`
- **Значение:** `<set-via-DYXLESS_TOKEN>`
...
1. Переменная окружения `DYXLESS_TOKEN` = `<set-via-DYXLESS_TOKEN>` добавлена (и есть fallback в коде).
